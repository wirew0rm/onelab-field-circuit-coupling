...
pnt1[] += newp; Point(newp) = { wcoreE,       htot/2-hcoreE+hcoil1+hcoil2, 0, lc0};
pnt1[] += newp; Point(newp) = { wcoreE+wcoil, htot/2-hcoreE+hcoil1+hcoil2, 0, lc0};

pnt2[] += newp; Point(newp) = { 0,              htot/2-hcoreE+hcoil1+hcoil2+wcoreE, 0, lc0};
pnt2[] += newp; Point(newp) = { 2*wcoreE+wcoil, htot/2-hcoreE+hcoil1+hcoil2+wcoreE, 0, lc0};

pnt1a[] += newp; Point(newp) = { wcoreE,       htot/2-hcoreE+hcoil1, 0, lc0};
pnt1a[] += newp; Point(newp) = { wcoreE+wcoil, htot/2-hcoreE+hcoil1, 0, lc0};
...
lnha[] += newl; Line(newl) = {pnt1a[0],pnt1a[1]};

lnv[] += newl; Line(newl) = {pnt0[0],pnt2[0]};
lnv[] += newl; Line(newl) = {pnt0[1],pnt1a[0]};
lnva[] += newl; Line(newl) = {pnt1a[0],pnt1[0]};
lnv[] += newl; Line(newl) = {pnt0[2],pnt1a[1]};
lnva[] += newl; Line(newl) = {pnt1a[1],pnt1[1]};
lnv[] += newl; Line(newl) = {pnt0[3],pnt2[1]};

Line Loop(newll) = {lnh[0],lnv[1],lnva[0],lnh[3],-lnva[1],-lnv[2],lnh[2],lnv[3],-lnh[4],-lnv[0]};
surf_ECore[] += news ; Plane Surface(news) = newll-1;

Line Loop(newll) = {lnh[1],lnv[2],-lnha[0],-lnv[1]};
surf_Coil[] += news ; Plane Surface(news) = newll-1;
Line Loop(newll) = {lnha[0],lnva[1],-lnh[3],-lnva[0]};
surf_Coil[] += news ; Plane Surface(news) = newll-1;
...
If(!Flag_Symmetry)
  ...
  surf_Coil[]   += Symmetry {1,0,0,0} { Duplicata{Surface{surf_Coil[1]};} };
